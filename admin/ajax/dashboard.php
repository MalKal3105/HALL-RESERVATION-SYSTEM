<?php
    ...
    if(isset($_POST['booking_analytics']))
    {
        ...
        // ada tarik table trans_amt dari db dekat LINE 16 & 19
        // ada guna table pending LINE 18
        $result = mysqli_fetch_assoc(mysqli_query($con,"SELECT 
            ...
            SUM(CASE WHEN booking_status!='pending' AND booking_status!='payment failed' THEN transfer_amt END) AS total_amt,

            COUNT(CASE WHEN booking_status!='booked' THEN 1 END) AS active_bookings,
            SUM(CASE WHEN booking_status!='booked' THEN transfer_amt END) AS active_amt,

            COUNT(CASE WHEN booking_status='cancelled' AND refund=1 THEN 1 END) AS cancelled_bookings,
            SUM(CASE WHEN booking_status='cancelled' AND refund=1 THEN transfer_amt END) AS cancelled_amt
