...
if (isset($_POST['upd_general'])) {
    ...
    // Use backticks (`) for table and column names, and question marks (?) for placeholders
...
if (isset($_POST['upd_shutdown'])) {
    // Use the value from $_POST['upd_shutdown']
    $frm_data = ($_POST['upd_shutdown'] == 0) ? 1 : 0;

    // Use backticks (`) for table and column names, and question marks (?) for placeholders
    $q = "UPDATE `settings` SET `shutdown`=? WHERE `sr_no`=?";

    // Provide all three values in the $values array
    $values = [$frm_data, 1]; // Assuming 'sr_no' should always be 1, adjust if necessary

    $res = update($q, $values, 'ii'); // Assuming 'shutdown' and 'sr_no' are integers
    echo $res;
}
